...
module temp_arrays_real
 ...
 real*8,  allocatable  :: qhat(:,:,:,:),qhat_ij(:,:,:,:)
 real*8,  allocatable  :: rhat(:,:,:,:),rhat_ij(:,:,:,:),s(:,:,:,:)
 integer, allocatable  :: qtarget(:),rtarget(:)
...
subroutine updateTS_real(nq,nvar,r,vol,rank,n,h,w0,tcomp,tcomm,jmax,kmax,lmax,timeComm)
  ...
  if (not_allocated_af.eq.1) then
     allocate(qhat(nq,jmax,kmax,lmax))
     allocate(rhat(nq,jmax,kmax,lmax))
     allocate(qhat_ij(nq,jmax,kmax,lmax))
     allocate(rhat_ij(nq,jmax,kmax,lmax))
     allocate(s(nq,jmax,kmax,lmax))
     allocate(dft_j(n))
     allocate(idft_j(n))
     allocate(kk(n),sk(n),qtarget(n),rtarget(n))
     !
     j=rank+1
     !
     rhs=0.0d0
